...
async function apiRequest(method, params = {}, credentials = null, usePost = false) {
  const config = credentials || (await getConfig());
  const fields = {
    ...(config.username ? { api_username: config.username } : {}),
    ...(config.apiPassword ? { api_password: config.apiPassword } : {}),
    method,
    content_type: "json",
    ...params,
  };
  ...
  try {
    if (usePost) {
      ...
      response = await fetch(API_URL, { method: "POST", body: form, redirect: "error", signal: controller.signal });
    } else {
      const url = new URL(API_URL);
      for (const [key, value] of Object.entries(fields)) {
        if (value !== undefined && value !== null && value !== "") url.searchParams.set(key, String(value));
      }
      response = await fetch(url.toString(), { method: "GET", cache: "no-store", redirect: "error", signal: controller.signal });
    }
  } catch (error) {
  ...
  try {
    ...
    throw new Error(`VoIP.ms returned ${response.status} and a non-JSON response.`);
  }
...
async function discoverPublicIp() {
  const data = await apiRequest("getIP", {}, { username: "", apiPassword: "" });
  return data.ip || data.public_ip || data.address || "Unknown";
}
...
async function fetchDids(credentials) {
  const data = await apiRequest("getDIDsInfo", {}, credentials);
...
async function publicState() {
  ...
  const config = await getConfig();
  return {
    config: { ...config, apiPassword: config.apiPassword ? "••••••••" : "" },
...
async function handleMessage(request) {
  switch (request.type) {
    case "GET_STATE":
      return publicState();
    case "GET_PUBLIC_IP":
      return { ip: await discoverPublicIp() };
    case "CONNECT": {
      const credentials = {
        username: String(request.username || "").trim(),
        apiPassword: String(request.apiPassword || ""),
      };
      if (!credentials.username || !credentials.apiPassword) throw new Error("Enter your API email and API password.");
      const dids = await fetchDids(credentials);
